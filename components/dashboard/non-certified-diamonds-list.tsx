...
import { Diamond, Edit, Eye, MoreHorizontal, ShoppingCart, Trash } from "lucide-react"
import Image from "next/image"

interface NonCertifiedDiamond {
  id: string
  image: string
  shape: string
  carat: number
...
interface NonCertifiedDiamondsListProps {
  view: "grid" | "table"
  diamonds: NonCertifiedDiamond[]
}

export function NonCertifiedDiamondsList({ view, diamonds }: NonCertifiedDiamondsListProps) {
  if (view === "grid") {
    return (
          ...
          <Card key={diamond.id} className="overflow-hidden">
            <CardHeader className="p-0">
              <div className="relative h-48 bg-gray-100">
                ...
                />
                <Badge className="absolute top-2 right-2 bg-white text-black">{diamond.quality}</Badge>
              </div>
            </CardHeader>
            <CardContent className="p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <Badge variant={diamond.parcel.includes("Parcel") ? "secondary" : "outline"}>{diamond.parcel}</Badge>
              </div>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-right">Parcel</TableHead>
        ...
        <TableBody>
          {diamonds.map((diamond) => (
            <TableRow key={diamond.id}>
              ...
              <TableCell>
                <Badge variant="outline">{diamond.quality}</Badge>
              </TableCell>
